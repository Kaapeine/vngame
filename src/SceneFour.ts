...
export class SceneFour extends Container implements IScene {
  ...
  private mealShown: boolean = false;
  private meal: Sprite;
  ...
  constructor() {
    ...
    let fire: AnimatedSprite = AnimatedSprite.fromImages([
  ...
  public addIngredients(): void {
    ...
    const spice: DragSprite = new DragSprite(
    ...
    const coconutmilk: DragSprite = new DragSprite(
    ...
    const dryfruit: DragSprite = new DragSprite(
    ...
    const rice: DragSprite = new DragSprite(
      'scene_four/Rice.png',
      this.mainContainer,
      pot,
    );
    ...
    const honey: DragSprite = new DragSprite(
  ...
  public update(_delta: number): void {
    ...
    if (this.ingreds && this.mainContainer.children.length === 10) {
      gsap.to(this.smoke, { alpha: 1, duration: 0.5 });
      this.ingreds = false;
    }

    if (!this.mealShown && this.mainContainer.children.length === 6) {
      this.mealShown = true;
      this.mainContainer.addChild(this.meal);
      gsap.to(this.meal, { alpha: 1, duration: 0.4 });
